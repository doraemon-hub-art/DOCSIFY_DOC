...
emplace 会直接在map内部调用构造函数，不会发生拷贝复制/构造。

移动构造和移动复制，需要配套出现。
- 移动构造，只需要接管即可；
- 移动复制，需要先清理自己原有的资源；

同样的一个类中的成员类，最好也支持移动构造。
...
# 4. vector push的顺序和遍历的顺序是否一致？

todo：

是的，

但是 cpp JSON底层用的类似于std::map的结构，用string存储的时间戳当做key，存储的时候，会进行字符串的比较，进行重新排序，而不是根据具体的数值，因为上电时间，从0开始，时间戳位数是很容易变化的。字符串比较排序，所以会乱序。
...
std::once_flag是C++11引入的同步原语。

同于确保某个特定任务（函数），在多线程环境下只被执行一次。

必须配合，std::call_once一起使用。
...
std::call_once(flag, call_function());
```
...
- 对于多个线程来说，效果类似于锁，当一个线程进入后，会阻塞其他线程；
- 并且，如果在call_once的执行函数中，有异常抛出，则会重置once_flag状态；
  - 另一线程可以再次执行；
  - 如果没有throw抛到外面，异常会被catch吞掉，不会重置once_flag状态；

在这个示例中，除去call_once用法，还有一些额外的注意事项，这里一起汇总讨论下。

- UB 和 异常
  - UB 即 Undefine Behavior，出现这个错误程序会直接退出；
    - 例如: 越界，空指针解引用；
  - 异常 则会在程序中抛出，如果有catch接收，则可以继续运行；
    - 选择throw继续抛出后，对应的调用地方也要有catch接收，否则程序也会直接退出；

---

> 补充

如果在明确的单线程环境下，则优先使用static或者使用简单的bool变量。

不建议使用std::once_flag，它内部包含了原子操作、同步语义，甚至可能涉及互斥锁。在单线程环境下，这些保护机制全是浪费。
...
# 8. 在高性能循环（如视频帧发送线程）中添加一次性判断的分支开销

> 什么是高性能循环？

“高性能循环”（High-Performance Loop）通常指代那些执行频率极高、时间敏感度极强、且处于程序运行路径核心的代码循环。

现代CPU 的分支预测，会在if条件永远不改变后将其优化掉，性能开销微乎其微。

---

> 代码案例: 

比如在发送视频帧的线程中，if条件在发送第一帧后永不成立。
...
string的size/length方法，计算的大小不包括结尾的结束符\0，只有里面的纯字符长度，如示例中所示，memcpy依此长度，最终只拷贝了数据，结尾没有字符串结束符。
...
非静态成员函数比普通函数多了一个隐藏的this指针参数，函数签名不同，不能混用。
