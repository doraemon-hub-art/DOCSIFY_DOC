...
- 位置状态: 通常安置在一个已知精确坐标，的固定位置上；
- 功能:
  - 观测卫星: 基站的GNSS天线会持续接收天空中的卫星信号（如GPS、北斗等）；
  - 计算误差: 由于卫星信号在穿透地球大气层（电离层、对流层）时会产生延迟，且卫星钟差、轨道误差等也会影响定位，基站会对比“卫星测出来的坐标”与“自身已知的精确坐标”。这两者之间的差值，就是当前区域的系统定位误差；
  - 发送修正数据: 基站将这些误差修正数据（差分数据），通过无线电台（电台模式）或互联网（网络模式），实时发送出去；

---

## 移动站（Rover）

是具体的作业工具，用户手持、安装在移动载体上的接收机。

- 位置状态: 处于移动状态；
- 功能: 
  - 接收双向信号: 移动站一边接收来自头顶的定位信号，一百年通过无线电或网络接收来自基站发送的误差修正数据； 
  - 实时差分求解：移动站内部的芯片和算法会将这两组数据进行实时差分处理。因为移动站与基站距离较近（通常在十几公里以内），它们穿过的电离层和对流层几乎相同，因此移动站可以通过基站的修正数据，将绝大部分的大气延迟和轨道误差消除；
  - 输出高精度坐标：完成差分计算后，移动站就能实时确定自己当前所处的厘米级精确位置；

---

> 总结

在实际应用中，基站和移动站缺一不可。它们之间必须保持通畅的通信链路（电台或网络），移动站才能实现“固定解”（即达到厘米级的高精度定位状态）。

---

## 应用场景

通过RTK系统获取到的是极度高精度、实时的三维地理坐标（纬度、经度、高度）

配合IMU、视觉、激光雷达，进行多传感器融合，让移动机器人（如智能机器人、自动驾驶汽车等）获得在任何复杂环境下的“高可靠性、高安全性、连续不断的**厘米级定位与环境感知**能力”。

---

# SLAM

SLAM 是 Simultaneous Localization and Mapping（即时定位与地图构建）的缩写。

简单来说，SLAM 解决的是这样一个问题：一个移动的机器人（或设备）在未知环境中，如何一边确定自身的位置（定位），一边构建环境的地图（建图）。
...
在原本我们的系统中并不强调系统软件&硬件时间的准确性，为了保护数据采集（算法），防止算法模块在运行后系统时间发生跳变引起错误，所以NTP服务也关闭了，但是接入网络摄像头后，连接信令服务器时，处于对安全性的考虑，对系统软件时间有着严格的限制，只能与真实的UTC时间相差±5分钟。

已有的同步时间逻辑，在系统启动后，系统脚本中进行GPS同步，当一但搜不到GSP信号，则无法更新。

后续更新时间，则是只能在系统第一次联网后，更新硬件时间，但是网络摄像头SDK底层获取的是软件时间。

- 一个简短的尝试。

在最开始，我尝试修改网络摄像头SDK源码的时间来源，使用第一次请求，保存当前硬件时间，后续每次请求时，基于当前上电时间作为偏移量，来计算当前时间。

经过验证后，结论是单纯修改网络摄像头SDK是不够，因为同样在TLS建链时，也会获取自己的时间戳，我们的修改，影响不到他，而修改BoringSSL底层依赖，又感觉大费周章，没有意义，最终只能放弃。

而强行更新系统软件时间，则会引起时间跳变，对已有的软件算法造成影响。
...
我们需要在各个硬件模块与上位机传递数据的合适时间点，注入我们新提供的接口——上电时间(CLOCK_MONOTONIC)，作为新的时间戳源头。

来让系统在启动后，能够不对其他模块引起时间跳变的基础上，更新正确的系统软件时间。
...
- 无法执行时钟来源，唯一的时钟来源是 CLOCK_REALTIME ，即系统软件时间，
- 精度为微妙；
...
## 帧间隔（duration / frame interval）

同话题两条连续消息的时间差（发布端发出消息的时间差）。

意义：衡量话题的发布稳定性。摄像头标称 30Hz → 帧间隔应该是稳定的 33ms。如果出现 50ms、100ms 的间隔，说明有抖动或丢帧。

| 话题类型 | 典型 Hz       | 典型帧间隔     |
| -------- | ------------- | -------------- |
| 摄像头   | 30Hz          | 33ms           |
| 激光雷达 | 10~20Hz       | 50~100ms       |
| IMU      | 100~400Hz     | 2.5~10ms       |
| 轮速     | 50~100Hz      | 10~20ms        |
| 控制指令 | 50~100Hz      | 10~20ms        |

---

## 方差（std / standard deviation）

方差衡量的是一组数据里的每个数偏离平均值的程度。

- 方差大 → 数值之间差异大，有的很大有的很小，很"散";
- 方差小 → 每个数值都差不多，很"集中";
...
帧率是指每秒显示的帧数，单位是 FPS（Frames Per Second），即"每秒多少帧"。

它衡量的是画面更新的速度。帧率越高，每秒画面刷新次数越多，动作看起来就越流畅、越连贯。

与帧间隔的关系：

- 帧率 = 1000 / 帧间隔（帧间隔单位是 ms）；
- 帧间隔 = 1000 / 帧率；
- 两者是倒数关系，描述的是同一件事的两个侧面:
  - 帧间隔：一帧一帧地看，看每两帧之间隔了多久;
  - 帧率：整秒地看，看一秒内能出现多少帧;
