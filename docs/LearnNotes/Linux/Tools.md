...
## Apple Magic Trackpad 在 Ubuntu 上不可用

（以下内容来自deepseek-v4-flash总结，其实不是苹果触控板，PDD买的一个没牌子的，仿的苹果，在设备列表中名字也改了。）

### 环境
- 设备：Apple Magic Trackpad (1代, 05ac:0265)
- 系统：Ubuntu 22.04, 内核 6.8.0-124-generic, X11
- 触控板数量：2 个（一个直连主板 USB 口，一个通过 Hub 连接）

### 根因（两层问题）

> 第一层 — Xorg 驱动冲突

- /etc/X11/xorg.conf.d/50-mtrack.conf 指定了 mtrack 驱动
- mtrack 使用废弃的 xf86OpenSerial() 打开设备，在现代 systemd-logind 环境下无权限
- 结果：驱动加载失败，触控板在 X11 中完全不可见

> 第二层 — 内核驱动 Bug

- hid-magicmouse 内核模块在 6.8.0 内核上有兼容性缺陷
- 能识别设备、能转发按键事件，但触摸/移动事件全被吞掉不向上层上报
- 导致 libinput 虽然正确加载了设备，但收不到任何触摸数据
...
│ 步骤 │ 操作                                    │ 作用                                         │
├──────┼─────────────────────────────────────────┼──────────────────────────────────────────────┤
│ 1    │ `50-mtrack.conf` 中 Driver 改为         │ 解决 mtrack 权限问题                         │
│      │ `libinput`                              │                                              │
├──────┼─────────────────────────────────────────┼──────────────────────────────────────────────┤
│ 2    │ 黑名单 `hid-magicmouse` 内核模块        │ 防止内核 Buggy 驱动抢设备，回退到            │
│      │                                         │ `hid-generic`                                │
├──────┼─────────────────────────────────────────┼──────────────────────────────────────────────┤
│ 3    │ 打开 Natural Scrolling                  │ 适配 macOS 滚动方向习惯                      │
└──────┴─────────────────────────────────────────┴──────────────────────────────────────────────┘

### 当前状态

- ✅ 两个触控板均可正常触摸、点击、滚动
- ✅ 重启后持久生效（通过 modprobe 黑名单）
- ⚠️ 根本修复需等待内核升级（hid-magicmouse 在 6.9+ 可能已修复）
